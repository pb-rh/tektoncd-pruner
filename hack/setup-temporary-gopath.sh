...
function shim_gopath() {
  ...
  local TEMP_OSP="${TEMP_GOPATH}/src/github.com/openshift-pipelines"
  local TEMP_TASKGROUP="${TEMP_OSP}/tektoncd-pruner"
  local NEEDS_MOVE=1
  ...
  # (repo-root)/.gopath/src/github.com/openshift-pipelines/tektoncd-pruner
  # to the user's pipeline checkout.
  ln -s "$REPO_DIR" "$TEMP_OSP"
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/openshift-pipelines/tektoncd-pruner"
